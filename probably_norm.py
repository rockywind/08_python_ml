    ...
    def forward(self, bottom, top):
        ...
        for n in range(bottom_data_shape[0]):
            ...
            for c in range(bottom_data_shape[2]):
                ...
                min_max_data = bottom_data[n,:, c, :]  / (np.sum(bottom_data[n,:, c, :]) + ep)
    ...
    def backward(self, top, propagate_down, bottom):
        ...
        for n in range(forward_top_data.shape[0]):
            for c in range(forward_top_data.shape[2]):
                diff_one_feature = np.zeros([feature_map_size, feature_map_size])
                #diff = 1 / (np.max(forward_top_data[n,:, c, :]) - np.min(forward_top_data[n,:, c, :]) + ep)
                for i  in range(feature_map_size):
                    for j  in range(feature_map_size):
                        diff_1 = (np.sum(forward_top_data[n,:, c, :]) - forward_top_data[n,i, c, j]) / (np.sum(forward_top_data[n,:, c, :])*np.sum(forward_top_data[n,:, c, :])+ep)
                        diff_one_feature[i,j] = diff_1
                diff = diff_one_feature
